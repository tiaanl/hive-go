...
func (c *container) Get(t reflect.Type) reflect.Value {
	...
	// If we can't find an exact match and we asked for an interface, then look in the container
	// for a value that implements the interface we're looking for.
...
func (c *container) Singleton(t reflect.Type, fn ValueFunc) TypeMapper {
